...
class AudioManager {
  ...
  updateToggleButton() {
    const toggle = document.getElementById('soundToggle');
    if (toggle) { toggle.textContent = this.enabled ? '🔊' : '🔇'; }
  }
  beep(frequency = 400, duration = 100) {
    if (!this.enabled) return;
    try {
  ...
  lose() { this.beep(400, 150); setTimeout(() => this.beep(300, 150), 100); }
  pickup() { this.beep(800, 50); setTimeout(() => this.beep(1000, 50), 50); }
  hit() { this.beep(300, 100); }
}
const audioManager = new AudioManager();    } catch (e) { consolaudio.jse.log('Audio not available'); }
  }
